...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (password_verify($password, $row['password'])) {  // Verifikasi password
            ...
            $_SESSION['nama'] = $row['nama'];  // Set session nama (jika ada di database)
            $_SESSION['role'] = $row['role'];  // Set session role
            
            // Redirect berdasarkan role
            if ($row['role'] == 'Admin') {
                header("Location: admin/layout.php");  // Arahkan ke dashboard admin
            } else if ($row['role'] == 'Sub Admin') {
                header("Location: subadmin/layout.php");  // Arahkan ke dashboard user
            } else {
                header("Location: user/layout.php");  // Arahkan ke dashboard user
            }
            exit();
        } else {
